...
export default function SalesPage() {
  ...
  /** When there is no landed cost, user enters selling price per unit (AMD). */
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Ընտրեք ապրանքը նկարի վրա՝ քանակ և գին մեկ պատուհանում, այնուհետև ավարտեք ամբողջ
            պատվերը։
          </p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {fruits.map((fruit) => (
                <button
                  ...
                  className="group relative aspect-square overflow-hidden rounded-xl border bg-muted text-left shadow-sm transition hover:ring-2 hover:ring-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
                  {fruit.image?.url ? (
                    <Image
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent px-2 pb-2 pt-8">
                    <p className="text-sm font-semibold text-white drop-shadow">{fruit.name}</p>
                    <p className="text-xs text-white/85">{formatFruitUnit(fruit.unit)}</p>
                  </div>
                </button>
              ))}
            </div>
                  ...
                  <div className="sticky bottom-0 border-t bg-background px-4 py-4 sm:px-6">
                    <div className="mb-3 flex items-center justify-between text-sm">
                      ...
                      <span className="text-lg font-semibold tabular-nums">
                        {effectiveDialogUnitPrice > 0
                          ? formatCurrency(dialogLineTotal)
                          : "—"}
                      </span>
                    </div>
                    <Button
